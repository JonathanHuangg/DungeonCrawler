...
public class Enemy4 extends Enemy implements EnemyInterface {
    public Enemy4(float x, float y, float attackDamage) {
    ...
    public void enemyMove(Difficulty difficultyLevel, float screenWidth, float screenHeight) {
        ...
        if (this.getEnemyX() < 0 || this.getEnemyX() > screenWidth) {
            this.setEnemyDx(-this.getEnemyDx());
            this.setEnemyX(Math.max(0, Math.min(this.getEnemyX(), screenWidth)));
        }
        if (this.getEnemyY() < 0 || this.getEnemyY() > screenHeight) {
